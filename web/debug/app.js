...
import { connectBle, flashFirmware, on, stats, DEFAULT_FW_URL, jitterQueue } from '../core.js';
...
on('connection', (state) => { connState.textContent = state; });
on('battery', (pct, chrg) => { batteryEl.textContent = `${pct}%${chrg ? ' (charging)' : ''}`; });
on('gate', (name) => { gateStateEl.textContent = name; });
on('flashProgress', (pct) => { flashProgress.value = pct; });
on('flashStatus', (msg) => { flashStatus.textContent = msg; log(`FLASH: ${msg}`); });
