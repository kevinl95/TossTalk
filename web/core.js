...
const DEFAULT_FW_URL = new URL('./firmware/tosstalk-merged.bin', import.meta.url).href;
...
let assemblyPred = 0, assemblyIdx = 0, assemblyMuted = false;
let lastCompleteSeq = null;

// Stats (readable by UIs)
export const stats = { frames: 0, drops: 0, mutedFrames: 0, concealedFrames: 0, subPkts: 0 };
...
function emit(event, ...args) {
  for (const fn of listeners[event] || []) { try { fn(...args); } catch {} }
}
...
function makeConceal(n) { return new Int16Array(n); }
...
function resetAssembly() {
  assemblySeq = null; assemblyPkts = 0;
  assemblyAdpcm = new Uint8Array(80);
  assemblyPred = 0; assemblyIdx = 0; assemblyMuted = false;
}
...
export async function connectBle() {
  if (isConnecting) { emit('log', 'Connect already in progress'); return; }
  let server = null;
  isConnecting = true;
  try {
    if (!('bluetooth' in navigator)) {
      emit('connection', 'Unavailable');
      alert('Web Bluetooth unavailable. Use Chrome or Edge desktop over HTTPS.');
      return;
    }
      ...
      try {
        ...
        for (let st = 1; st <= 3; st++) {
          ...
          try {
            ...
            if (!device.gatt.connected) {
              emit('log', '⚠️ Connection lost. Try power-cycling the device and reconnecting.');
...
export async function flashFirmware({ firmwareUrl, firmwareFile, flashAddress = 0, eraseAll = false, logFn }) {
  ...
  try {
    ...
    let fw;
    if (firmwareFile) {
      fw = { name: firmwareFile.name, data: new Uint8Array(await firmwareFile.arrayBuffer()) };
    } else {
    ...
    if (eraseAll && loader.IS_STUB) { emit('flashStatus', 'Erasing...'); await loader.eraseFlash(); }
